...
export default function PlayVsEngine({ playerColor, boardTheme, onExit }) {
  ...
  const boardSize = 'clamp(280px, 55vw, 520px)';
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-4 md:p-8">
      ...
      {/* Board + Side panel */}
      <div className="flex flex-col lg:flex-row gap-4 items-start w-full max-w-4xl">
        {/* Board */}
        ...
        {/* Side panel */}
        <div className="flex-1 min-w-0 space-y-3 w-full slide-in-right">
          {/* Status */}
          ...
          <div className="rounded-xl p-3" style={{ background: 'rgba(15,20,40,0.6)', border: '1px solid rgba(107,140,174,0.08)' }}>
            ...
            <div className="flex gap-1.5">
              {SKILL_LEVELS.map(s => (
                <button key={s.level} onClick={() => setSkillLevel(s.level)} className="flex-1 py-1.5 rounded-lg transition-all hover:scale-105 text-center" style={{
